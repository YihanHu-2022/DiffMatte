...
class Swin(Backbone):
    ...
    def forward(self, x, sampleidx=None):
        trimapmask = x[:, 3:, :, :]
        # set outputs
        outs = []
        ...
        for i in range(self.num_layers):
            ...
            #     out = x_out.view(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous()
                ...
                # outs.append(out)
        out = x_out.view(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous()
        # return tuple(outs) # 增加一层upsample
        return out # 增加一层upsample
...
if __name__ == '__main__':
    ...
    # out = model(torch.ones(2, 6, 512, 512).cuda(), torch.ones(2,3,512,512).cuda()) # (image+trimap, trimap)
    out = model(torch.ones(2, 6, 1024, 1024).cuda(), torch.ones(2,3,1024,1024).cuda()) # (image+trimap, trimap)
    print(out.shape)
    # print(len(out))
    ...
    """
    6
    torch.Size([2, 768, 16, 16]) # input 512 x 512
    torch.Size([2, 768, 32, 32]) # input 1024 x 1024
    """
    print('MODEL DEBUG')